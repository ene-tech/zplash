...
import { buscarClientesPorTelefono } from "@/lib/dataAccess/clientes";
...
import { formatTelefono, isValidTelefono } from "@/lib/helpers";
...
export async function POST(request: NextRequest) {
  ...
  let body: { telefono?: unknown; codigo?: unknown };
  try {
  ...
  const telefono = formatTelefono(typeof body.telefono === "string" ? body.telefono.trim() : "");
  const codigo = typeof body.codigo === "string" ? body.codigo.trim() : "";
  if (!isValidTelefono(telefono) || !telefono || !/^\d{6}$/.test(codigo)) {
  ...
  try {
    [fila] = await db
      .select()
      .from(otpsCliente)
      .where(and(eq(otpsCliente.telefono, telefono), isNull(otpsCliente.usadoEn)))
  ...
  const clientesEncontrados = await buscarClientesPorTelefono(telefono);
  if (!clientesEncontrados.length) {
    return NextResponse.json({ ok: false, error: "Este teléfono ya no está asociado a ningún vehículo" }, { status: 404 });
  }
