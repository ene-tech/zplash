...
export async function crearSesion(perfil: { id: string; nombre: string; modulos: Modulo[] }): Promise<void> {
  const payload: SesionPayload = { id: perfil.id, nombre: perfil.nombre, modulos: perfil.modulos, exp: Date.now() + DURACION_MS };
